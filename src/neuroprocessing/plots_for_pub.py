...
def plot_STA_img_and_dff(
    ...
    **dff_plot_kwargs,
    ...
    _, t, dff = imaging_trial.get_sta_stack(1, 5, 0.5, trial_ROI)
    sta_df, _, _ = imaging_trial.get_sta_stack(1, 5, 0.5)
    ...
    if bleach_subtract:
        dff = dff - np.polyval(
            np.polyfit(
                np.array([t[:3], t[-3:]]).flatten(), np.array([dff[:3], dff[-3:]]).flatten(), 1
            ),
            t,
        )
    ...
    if show_montage:
        ...
        _, axs = plt.subplots(1, 3, figsize=(18, 6), width_ratios=[0.45, 0.02, 0.35])  # 6.5
...
def plot_montage_and_trace_pairs(
    imaging_trials,
    colors,
    ...
    for trial, ax, color in zip(imaging_trials, axs.T, colors, strict=True):
        # montage plot
        montage_stack = trial.plot_montage(
            s_start=0,
            s_end=1200,
            s_step=MONTAGE_STEP,
    ...
    axs[1, 0].set_xlabel(
        "Time (s)", fontname=plt.rcParams["font.sans-serif"][0], fontweight="medium"
    )
    axs[1, 1].set_xlabel(
        "Time (s)", fontname=plt.rcParams["font.sans-serif"][0], fontweight="medium"
    )
