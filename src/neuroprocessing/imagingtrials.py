...
class ImagingTrial:
    ...
    * `mask_... .npy` - ROI mask
    ...
    def _get_roi_mask(self, dims: tuple, roi: dict):
    ...
    def load_trace(self, roi=None):
        """
        Returns time vector (in seconds) and trace vector of whole-brain activity
        (if ROI not defined) or ROI-bounded activity (if ROI dict is defined) for the trial.
        ...
        t = (np.arange(0, len(trace))) / (
            self.sync_info["framerate_hz"] / self.params["downsample_factor"]
        ) - self.params["secs_before_stim"]
        return t, trace
    ...
    def plot_montage(
        self, s_start: int, s_end: int, s_step=1, montage_grid_shape=None, ax=None, **plot_kwargs
    ) -> np.ndarray:
        """
    ...
    def get_sta_stack(self, s_pre_stim=1, s_post_stim=5, s_baseline=0.5, roi=None):
        ...
        Inputs:
            ...
            s_baseline: int
                Number of seconds before stimulus onset to use as baseline for
        ...
        Returns:
            sta_df_stack: np.ndarray
                Stimulus-triggered, background-subtracted stack [n_trials x n_frames x h x w]
                for the trial.
        ...
        frame_pre_stim, frame_post_stim = (
            self._seconds_to_frame_num(s) for s in [s_pre_stim, s_post_stim]
        )
        ...
        if roi is None:
            sta_stack = np.stack(
                [
                    processed_stack[sof - frame_pre_stim : sof + frame_post_stim, :, :]
                    for sof in stim_onsets_downsampled[1:-1]
                ]
            )
        else:
            ...
            sta_stack = np.stack(
                [
                    processed_stack[sof - frame_pre_stim : sof + frame_post_stim, roi_mask]
                    for sof in stim_onsets_downsampled[1:-1]
                ]
            )

            # reshape stack back to [n_trials x n_frames x h_roi x w_roi]
            sta_stack = sta_stack.reshape(
                (sta_stack.shape[0], n_frames, roi["height"], roi["width"])
            )

        frames_baseline = self._seconds_to_frame_num(s_baseline)
        sta_baseline_mean = sta_stack[:, :frames_baseline, :, :].mean(
            axis=1
        )  # get first frames_baseline frames and average them
        sta_df_stack = (
            sta_stack - sta_baseline_mean[:, np.newaxis, :, :]
...
class ImagingTrialsLoader:
    ...
    def __init__(self, base_path):
        ...
        self.trials = [
            ImagingTrial(base_path, e, t) for e, t in zip(exp_dirs, trial_dirs, strict=True)
        ]
