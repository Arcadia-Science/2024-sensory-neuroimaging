...
date_option = click.option(
    "--date", type=str, required=True, help="Experiment date folder to analyze."
)

params_file_option = click.option(
    "--params_file",
    type=click.Path(exists=True, file_okay=True, dir_okay=False, path_type=Path),
    required=True,
    help="Path to JSON file containing analysis parameters. See README.md for details.",
)

trial_option = click.option(
    "--trial",
    type=str,
    required=False,
    help=(
        "Trial folder to analyze. If not provided, all trials in the experiment "
        "folder will be analyzed."
    ),
)

reanalyze_option = click.option(
    "--reanalyze",
    is_flag=True,
    help=(
        "If set, reanalyze all trials, even if already processed. "
...
def execute_pipeline(date, trial, params_file, reanalyze):
    """Runs analysis pipeline (preprocess and process)."""
    ...
    exp_path = Path(config["raw_data_dir"]) / date
